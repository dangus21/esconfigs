...
function installDeps(manager, config, withTailwind = false) {
  const currDirFiles = (0, import_fs.readdirSync)(path);
  ...
  if (currDirFiles.length === 0 || currDirFiles.length > 0 && !currDirFiles.includes("package.json")) {
    ...
    (0, import_child_process.spawnSync)(currDirPackageManager, ["init", "-y"], spawnOptions);
  }
  if (manager === "current") {
...
(() => __async(void 0, null, function* () {
  const { configType } = yield (0, import_prompts.prompt)({
    type: "multiselect",
    name: "configType",
  ...
  const { withTailwind } = yield (0, import_prompts.prompt)({
    name: "withTailwind",
